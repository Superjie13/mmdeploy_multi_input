...
Author: Sijie Hu
Date: 20/03/2024
Description: This script adapted from mmdeploy deploy.py to set the input `img` as an `img-pair`.
specifically:
    ln 33: parser.add_argument('img-pair', help='image pair used to convert model model, input like: "path1, path2"')
    ln 34: parser.add_argument('--test-img_pair', default=None, type=str, nargs='+', help='image pair used to test model')
    ln 151: img_pair = [img.strip() for img in args.img_pair.split(',')] -> ln 153
"""
import argparse
import logging
import os
...
from mmdeploy.apis import (create_calib_input_data, extract_model,
                           get_predefined_partition_cfg, torch2onnx_mm,
...
def torch2ir(ir_type: IR):
    ...
    """
    if ir_type == IR.ONNX:
        return torch2onnx_mm
    elif ir_type == IR.TORCHSCRIPT:
        raise NotImplementedError('TorchScript is not supported now.')
    else:
...
def main():
    ...
    pipeline_funcs = [
        torch2onnx_mm, torch2torchscript, extract_model, create_calib_input_data
    ]
    ...
    img_pair = [img.strip() for img in args.img_pair.split(',')]
    img_pairs = [img_pair]
    torch2ir(ir_type)(
        img_pairs,
        args.work_dir,
    ...
    if backend == Backend.RKNN:
        ...
        with open(dataset_file, 'w') as f:
            f.writelines([osp.abspath(img_pair[0]), osp.abspath(img_pair[1])])
